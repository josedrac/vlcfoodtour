...
if(!function_exists('dt_contact_form')) {
	...
	function dt_contact_form( $atts, $content = null ) {
		extract(shortcode_atts(array(
			...
			'success_msg' => __('Thank you for your message. I will get back to you asap.', 'iamd_text_domain'),
			'error_msg' => __('Sorry your message not sent, Try again Later.', 'iamd_text_domain')
		), $atts));

		$name_label = __('Name:', 'iamd_text_domain');
		$message_label = __('Message:', 'iamd_text_domain');
		$email_label = __('Name:', 'iamd_text_domain');
		...
		$out .= '<form class="wpcf7-form contact-frm" method="post" action="'.get_template_directory_uri().'/framework/sendmail.php">';
			$out .= '<div class="dt-sc-one-half column first">';
				...
				$out .= '<p class="dt-p-captcha"><span class="span-label">'.__('Captcha:', 'iamd_text_domain').'</span> <span class="dt-sc-captcha">'.$temp[0].'<sup>'.$temp[1].'</sup>'.$temp[2].'<sub>'.$temp[3].'</sub></span>
						 </p>';
				$out .= '<p class="dt-p-captcha">
							<!--<span class="label">Type the number in the box:</span>-->
							<input type="text" name="txtcap" placeholder="'.__('Type the number in the box *', 'iamd_text_domain').'" required="required" />
							<input type="hidden" id="txthidcap" value="'.$ctemp.'" readonly>
						 </p>';
			$out .= '</div>';

			$out .= '<p class="dt-sc-one-half column"><textarea cols="12" rows="12" name="cmessage" placeholder="'.__('Message *', 'iamd_text_domain').'"></textarea>
			...
			$out .= '<input type="hidden" name="hidnamelabel" value="'.$name_label.'" />';
			$out .= '<input type="hidden" name="hidmessagelabel" value="'.$message_label.'" />';
			$out .= '<input type="hidden" name="hidemaillabel" value="'.$email_label.'" />';
